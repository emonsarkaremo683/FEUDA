    ...
    <section className="max-w-7xl mx-auto px-4 py-16">
      <div className="flex flex-col sm:flex-row items-center justify-between mb-12 gap-6">
        <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 text-center sm:text-left">
          We Are Committed To Creating <br/> A Better World
        </h2>
        
        {/* Auto Play Toggle */}
        <div className="flex items-center gap-3 bg-gray-100 p-2 rounded-2xl">
          <span className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-2">Auto Play</span>
          <button 
            onClick={() => setAutoPlay(!autoPlay)}
            className={`w-12 h-6 rounded-full transition-all relative ${autoPlay ? 'bg-orange-500' : 'bg-gray-300'}`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stories.map((story, index) => (
          <div key={story.id} className="relative aspect-[9/16] rounded-3xl overflow-hidden group cursor-pointer bg-slate-100">
            <video
            ...
            <div className="absolute top-4 left-4">
              {story.id === 1 && (
                <div className="text-white text-xs font-bold font-serif italic">
                  They won't remember the gifts, but <br/>
